...
export default function ManageResidentsScreen({ navigation }: any) {
  ...
  const loadResidents = async () => {
    try {
      ...
      if (Platform.OS === 'web') {
        window.alert('לא ניתן לטעון את רשימת הדיירים');
      } else {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        window.alert('פרטי הדייר עודכנו בהצלחה');
      } else {
      ...
      if (Platform.OS === 'web') {
        window.alert('לא ניתן לעדכן את פרטי הדייר');
      } else {
  ...
  const handleDeleteResident = (resident: Resident) => {
    const doDelete = async () => {
      try {
        ...
        if (Platform.OS === 'web') {
          window.alert('הדייר נמחק בהצלחה');
        } else {
        ...
        if (Platform.OS === 'web') {
          window.alert('לא ניתן למחוק את הדייר');
        } else {
    ...
    if (Platform.OS === 'web') {
      if (window.confirm(`האם אתה בטוח שברצונך למחוק את ${resident.name}?`)) {
        doDelete();
      }
    } else {
      Alert.alert(
  ...
  const handleToggleActive = async (resident: Resident) => {
    try {
      ...
      if (Platform.OS === 'web') {
        window.alert('לא ניתן לעדכן את סטטוס הדייר');
      } else {
