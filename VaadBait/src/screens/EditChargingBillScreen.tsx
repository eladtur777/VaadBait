...
export default function EditChargingBillScreen({ navigation, route }: any) {
  ...
  const handleSave = async () => {
    if (!currentReading || parseFloat(currentReading) <= parseFloat(previousReading)) {
      if (Platform.OS === 'web') {
        window.alert('הקריאה הנוכחית חייבת להיות גדולה מהקריאה הקודמת');
      } else {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        window.alert('החשבון עודכן בהצלחה');
        navigation.goBack();
      } else {
      ...
      if (Platform.OS === 'web') {
        window.alert('לא ניתן לשמור את החשבון');
      } else {
  ...
  const handleDelete = async () => {
    const doDelete = async () => {
      ...
      try {
        ...
        if (Platform.OS === 'web') {
          window.alert('החשבון נמחק בהצלחה');
          navigation.goBack();
        } else {
        ...
        if (Platform.OS === 'web') {
          window.alert('לא ניתן למחוק את החשבון');
        } else {
    ...
    const confirmMsg = 'האם אתה בטוח שברצונך למחוק חשבון זה? פעולה זו אינה ניתנת לביטול.';
    if (Platform.OS === 'web') {
