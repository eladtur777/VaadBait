...
exports.getDebtSummary = exports.sendDebtRemindersManual = exports.sendWeeklyDebtReminders = void 0;
...
function getEmailConfig() {
    ...
    return {
        service: ((_a = config.email) === null || _a === void 0 ? void 0 : _a.service) || process.env.EMAIL_SERVICE || 'gmail',
        user: ((_b = config.email) === null || _b === void 0 ? void 0 : _b.user) || process.env.EMAIL_USER || '',
        pass: ((_c = config.email) === null || _c === void 0 ? void 0 : _c.pass) || process.env.EMAIL_PASS || ''
    };
}
// Create transporter
...
exports.sendWeeklyDebtReminders = functions
...
exports.sendDebtRemindersManual = functions
    .runWith({ timeoutSeconds: 540, memory: '512MB' })
    ...
    if (!authHeader || authHeader !== `Bearer ${(_a = functions.config().api) === null || _a === void 0 ? void 0 : _a.secret}`) {
    ...
    try {
        ...
        let emailsSent = 0;
        for (const debt of debts) {
            if (!debt.resident.email || debt.totalDebt <= 0)
                continue;
            try {
                const emailHtml = generateDebtEmailHtml(debt);
                await transporter.sendMail({
...
async function collectAllDebts() {
    ...
    const residentsSnapshot = await db.collection('residents')
        .where('isActive', '==', true)
        .get();
    const residents = residentsSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
    // Get all unpaid fee payments (current month and past)
    const feePaymentsSnapshot = await db.collection('feePayments')
        .where('isPaid', '==', false)
        .get();
    const feePayments = feePaymentsSnapshot.docs
        .map(doc => (Object.assign({ id: doc.id }, doc.data())))
        .filter(fp => {
        // Only include current month and past (not future)
        if (fp.year < currentYear)
            return true;
        if (fp.year === currentYear && fp.month <= currentMonth)
            return true;
        return false;
    });
    ...
    const stations = stationsSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
    // Get all unpaid meter readings (current month and past)
    const meterReadingsSnapshot = await db.collection('meterReadings')
        .where('isPaid', '==', false)
        .get();
    const meterReadings = meterReadingsSnapshot.docs
        .map(doc => (Object.assign({ id: doc.id }, doc.data())))
        .filter(mr => {
        if (mr.year < currentYear)
            return true;
        if (mr.year === currentYear && mr.month <= currentMonth)
            return true;
        return false;
    });
    ...
    for (const resident of residents) {
        ...
        const residentStations = stations.filter(s => s.apartmentNumber === resident.apartmentNumber);
...
function generateDebtEmailHtml(debt) {
